...
def dc_token():
    api_key = os.getenv("KHULNASOFT_CLOUD_API_KEY", None)
    if not api_key:
        raise Exception("KHULNASOFT_CLOUD_API_KEY env var is not set")
    return api_key


async def upload_to_dc(workspace: str, file: Path, meta: Dict):
    url = f"https://api.cloud.khulnasoft.com/api/v1/workspaces/{workspace}/files"
...
if __name__ == "__main__":
    workspace = os.getenv("KHULNASOFT_CLOUD_WORKSPACE", None)
    if not workspace:
        print("KHULNASOFT_CLOUD_WORKSPACE env var not set")
        sys.exit(1)

    if "KHULNASOFT_CLOUD_API_KEY" not in os.environ:
        print("KHULNASOFT_CLOUD_API_KEY env var not set")
        sys.exit(1)
